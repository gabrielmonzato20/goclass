...
func (suite *OrderRepositoryTestSuite) TestGivenAnOrder_WhenList_ThenShouldReturnOrders() {
	...
	err = repo.Save(order)
	suite.NoError(err)
	order2, err := entity.NewOrder("1234", 100.0, 20.0)
	suite.NoError(err)
	...
	suite.Equal(orders[0].ID, order.ID)
	suite.Equal(orders[0].Price, order.Price)
	suite.Equal(orders[0].Tax, order.Tax)
	suite.Equal(orders[0].FinalPrice, order.FinalPrice)
